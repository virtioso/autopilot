...
zenoh-python backs `adapters/vcmux.py` — the VCMux Source oracle that discovers seL4 VM guest consoles dynamically and adds them as named BiStreams to StreamContext. VMs come up asynchronously after board boot; the oracle subscribes to a wildcard key expression and receives notifications as each VM console key goes live.
...
Zenoh is not broker-mandatory. Autopilot operates in **client mode**, connecting to a `zenohd` router running on the Orin AGX side (the Zenoh bridge to `virtioso-muxd`).

Zenoh is **message-boundary-preserving**: each `publisher.put(payload)` results in exactly one `Sample` at every subscriber. Bytes from `virtioso-muxd` arrive chunked at pty-read granularity (~line or read-buffer size). Concatenate chunks in order to reconstruct the byte stream — no special framing needed beyond that.
...
At 115200 bps (~11.5 KB/s), a serial console is roughly 0.0001% of Zenoh's measured throughput ceiling (67 Gbps on loopback). There are no throughput concerns at this data rate. The main practical consideration: if `virtioso-muxd` calls `put()` per byte or per character, sample overhead dominates. Better to publish per pty read-buffer (~64+ bytes). At 115200 bps this naturally produces ~12 bytes/ms per read — negligible.
