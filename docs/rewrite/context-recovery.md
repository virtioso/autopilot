...
│   ├── relay.py           # RelayOracle — USB relay board control via usbrelay_py
│   └── uefi.py            # UEFIShellRunOracle + ExtlinuxBootOracle
├── model/
...
1. **All steps 3–13 complete. 230 tests pass** (6 hardware/RF skipped).
   - `engine/`: oracle.py, combinators.py, recorder.py, primitives.py (+ FilterBiStream), runtime.py (+ RecordedOracle)
   - `adapters/`: uart.py, ssh.py, process.py, docker.py, vcmux.py, robot.py, interactive.py, relay.py, uefi.py
   - `model/chain.py`: Pydantic OracleDef (24 types incl. retry_errors on repeat_poll) + OracleFactory
   - `model/config.py`: layered Config (defaults → platform YAML → env → request)
   - `autopilot.py`: CLI with structlog/stdlib logging bridge; `run.log` per-run
   - `daemon.py`, `client.py`, `mcp_server.py`: asyncio daemon + MCP
   - `platforms/`: orin-agx.yaml, qemu-generic.yaml
   - `chains/` (33 migrated, Tier 0–3): all chains except Tier 4 deferred
   - `results/<run_id>/streams/<name>.raw`: universal raw stream capture via TeeStream + ctx.add_stream()
2. **Hardware validated (2026-05-24):** `sel4test.json` ran end-to-end on Orin AGX. Chain infrastructure confirmed working. sel4test binary itself has an unrelated BIND0001 vm fault — not a chain issue.
3. **Tier 4 deferred**: `prepare_next_run_task.json` (needs `signal_wait`), `isengard-linux-orin-zenoh-demo.json` (Zenoh-specific).
4. **Next action**: production cutover — swap `autopilot.py` entrypoint, update `PATH`.

**FilterBiStream** (engine/primitives.py): wraps any BiStream, strips ANSI CSI codes and normalises CRLF→LF. Enabled by default on all source oracles (uart_source, spawn_process, docker_container, vcmux_source) via `preprocess: bool = True`.

**Environment variables required for sel4test hardware run:**
```bash
